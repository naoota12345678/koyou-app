...
type Props = {
  ...
  allContracts: { status: string; weeklyHours: number }[];
  onClose: () => void;
};
...
export default function ContractForm({ user, departments, allContracts, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    const dept = departments.find((d) => d.id === form.companyId);
    if (!dept) return;
    setForm((p) => ({
      ...p,
      startHour: dept.defaultStartHour || p.startHour,
      startMinute: dept.defaultStartMinute || p.startMinute,
      endHour: dept.defaultEndHour || p.endHour,
      endMinute: dept.defaultEndMinute || p.endMinute,
      weeklyHours: dept.defaultWeeklyHours || p.weeklyHours,
      payClosingDay: dept.payClosingDay || p.payClosingDay,
      paymentDay: dept.paymentDay || p.paymentDay,
      increment: dept.incrementDefault ?? p.increment,
      bonus: dept.bonusDefault ?? p.bonus,
      retirementAllowance: dept.retirementAllowanceDefault ?? p.retirementAllowance,
      workplaceInitial: dept.address || p.workplaceInitial,
    }));
  }, [form.companyId, departments]);
  ...
  const handleSave = async () => {
    if (!form.companyId || !form.employeeName) {
      alert("部署と氏名は必須です");
      return;
    }
    const data = {
  ...
  const stepTitles = ["基本情報", "雇用形態・契約期間", "就業情報", "労働時間", "賃金・保険"];
  ...
  return (
    ...
    >
      <div style={{ background: C.white, borderRadius: 12, padding: 32, width: 600, maxHeight: "85vh", overflow: "auto" }}>
        {/* ステップインジケーター */}
        ...
        {step === 1 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <SelectField
              ...
              onChange={(v) => f("companyId", v)}
              options={[
                { value: "", label: "選択してください" },
                ...departments.map((d) => ({ value: d.id, label: d.name })),
              ]}
            />
            <FormField label="従業員氏名 *" value={form.employeeName} onChange={(v) => f("employeeName", v)} />
            <div>
              <div style={{ fontSize: 12, fontWeight: 600, color: "#555", marginBottom: 4 }}>発行日（令和）</div>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <SmallInput value={form.issueDateYear} onChange={(v) => f("issueDateYear", v)} width={50} />
                <span style={{ fontSize: 13 }}>年</span>
                <SmallInput value={form.issueDateMonth} onChange={(v) => f("issueDateMonth", v)} width={50} />
                <span style={{ fontSize: 13 }}>月</span>
                <SmallInput value={form.issueDateDay} onChange={(v) => f("issueDateDay", v)} width={50} />
            ...
            {form.isYuki ? (
              <>
                ...
                <SelectField
                  ...
                  onChange={(v) => f("renewalType", v)}
                  options={[
                    { value: "自動更新", label: "自動更新" },
                    { value: "更新あり", label: "更新あり" },
                    { value: "更新なし", label: "更新なし" },
                    { value: "その他", label: "その他" },
                  ]}
                />
              </>
              ...
              <FormField
                label="試用期間（月数）"
                value={String(form.trialPeriodMonths)}
                onChange={(v) => f("trialPeriodMonths", Number(v) || 0)}
                type="number"
              />
            )}
            <RadioGroup
        ...
        {step === 3 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <SelectField
              ...
              onChange={(v) => f("studentType", v)}
              options={[
                { value: "学生でない", label: "学生でない" },
                { value: "昼間学生", label: "昼間学生" },
                { value: "夜間・通信・定時制", label: "夜間・通信・定時制" },
              ]}
            />
            <SelectField
              label="採用経路"
              value={form.recruitmentSource}
              onChange={(v) => f("recruitmentSource", v)}
              options={[
                { value: "直接", label: "直接応募" },
                { value: "ハローワーク", label: "ハローワーク" },
                { value: "紹介", label: "紹介" },
                { value: "その他", label: "その他" },
              ]}
            />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ display: "flex", gap: 16 }}>
              <FormField
                ...
                onChange={(v) => f("weeklyHours", Number(v) || 0)}
                type="number"
              />
              <FormField
                label="週所定労働日数"
                value={String(form.weeklyDays)}
                onChange={(v) => f("weeklyDays", Number(v) || 0)}
                type="number"
              />
            </div>
            <RadioGroup
              label="副業"
              value={form.sideJobPolicy}
              onChange={(v) => f("sideJobPolicy", v)}
              options={[
                { value: "禁止", label: "禁止" },
                { value: "許可", label: "許可" },
                { value: "届出制", label: "届出制" },
            ...
            <CheckField
              label="テレワーク勤務あり"
              checked={form.teleworkAllowed}
              onChange={(v) => f("teleworkAllowed", v)}
            />
          </div>
        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {form.salaryType === "monthly" ? (
              <FormField
                ...
                onChange={(v) => f("basicSalary", Number(v) || 0)}
                type="number"
              />
            ) : (
              <FormField
                label="時給（円）"
                value={String(form.hourlyWage || "")}
                onChange={(v) => f("hourlyWage", Number(v) || 0)}
                type="number"
              />
            )}
            <div style={{ display: "flex", gap: 16 }}>
              <FormField
                label="固定残業手当（円/月）"
                value={String(form.fixedOvertimeAmount || "")}
                onChange={(v) => f("fixedOvertimeAmount", Number(v) || 0)}
                type="number"
              />
              <FormField
                label="固定残業（時間/月）"
                value={String(form.fixedOvertimeHours || "")}
                onChange={(v) => f("fixedOvertimeHours", Number(v) || 0)}
                type="number"
              />
            </div>
            <FormField
              label="通勤手当（円/月）"
              value={String(form.commuteAllowance || "")}
              onChange={(v) => f("commuteAllowance", Number(v) || 0)}
              type="number"
            />
            ...
            <div style={{ borderTop: `1px solid ${C.light}`, paddingTop: 14, marginTop: 4 }}>
              ...
              <div style={{ display: "flex", gap: 24, marginBottom: 8 }}>
                <div style={{ fontSize: 13 }}>
                  社会保険: <strong style={{ color: finalSocial ? C.green : C.red }}>{finalSocial ? "加入" : "非加入"}</strong>
                </div>
                <div style={{ fontSize: 13 }}>
                  雇用保険: <strong style={{ color: finalEmployment ? C.green : C.red }}>{finalEmployment ? "加入" : "非加入"}</strong>
                </div>
                <div style={{ fontSize: 12, color: C.gray }}>
                  特定適用: {isTokutei ? "該当" : "非該当"}
                </div>
              </div>
              <CheckField
                label="判定結果を手動で変更する"
                checked={form.socialInsuranceOverride}
                onChange={(v) => { f("socialInsuranceOverride", v); f("employmentInsuranceOverride", v); }}
              />
